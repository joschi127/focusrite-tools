...
- **Hardware Safety:** Ensure that hardware NVRAM flashing (`flash_hardware`) is only performed when explicitly requested (e.g., during standalone mode switching) to preserve hardware longevity.
- **Clean Deployment:** Use automated installation via `install.py` to handle EXE compilation (PyInstaller) and Windows Task Scheduler registration systematically.

## Documentation & Changelog
- **Context Awareness:** When working on tasks, always consider information from `ROADMAP.md` and `CHANGELOG.md` to ensure consistency with planning and past changes.
